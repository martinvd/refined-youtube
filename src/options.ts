...
const collapseHiddenElementsCheckbox = document.getElementById(
...
if (!(collapseHiddenElementsCheckbox instanceof HTMLInputElement)) {
  throw new Error(
    'Expected #collapse-hidden-elements to be an HTMLInputElement',
  );
}

void chrome.storage.sync
  .get({ hideShorts: true, collapseHiddenElements: true })
  .then((stored) => {
    const hideShorts =
      typeof stored.hideShorts === 'boolean' ? stored.hideShorts : true;
    const collapseHiddenElements =
      typeof stored.collapseHiddenElements === 'boolean'
